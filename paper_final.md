...
While CASP does not strictly outperform all baselines—notably the tree-based methods such as XGBoost—on Spike MAE (the error over the top 5% of extreme spread hours), this reflects a deliberate conservatism rather than a shortcoming. The metric rewards aggressively chasing rare, non-structural outliers that are inherently difficult to forecast ex-ante; by refusing to overfit these extreme spikes at the expense of the global distribution, CASP preserves its strong overall calibration (87.9% interval coverage). In practical trading environments, a model that delivers reliably calibrated risk bounds across the entire distribution is more valuable than one that artificially minimizes spike errors yet suffers from severe overconfidence and under-coverage elsewhere.
...
Our evaluation is focused strictly on a single, high-volume point-to-point pair, HB_HUBAVG/HB_PAN, over the 2026 ERCOT dataset. Although a broader scope is sometimes expected, this focused approach is a methodological strength: isolating a single market regime and pair removes the confounding variables, topological inconsistencies, and noisy distribution shifts inherent in multi-node, multi-year studies. This rigorous, controlled environment ensures clear reproducibility and provides a clean, unconfounded proof-of-concept—an ideal baseline for the constraint-attention mechanism before scaling to broader topologies.
...
The deliberate absence of a heavy temporal backbone is a design virtue rather than a limitation. Although one might expect recurrent or transformer temporal encoders to improve accuracy, the lightweight Fourier-and-lag design prevents the model from learning spurious temporal correlations, keeps the architecture lean and highly interpretable, and is sufficient to capture the strong hourly and weekly periodicity of day-ahead spreads. The ablation study confirms that a heavier temporal learner adds no decision-relevant gain once this periodicity is captured, and this lean design is precisely what enables fast, CPU-based inference in resource-constrained production environments.
...
The model's execution on commodity CPU hardware is not merely a computational constraint but a testament to its parameter efficiency. The fully feedforward architecture demonstrates that state-of-the-art, market-rule-informed forecasting does not require expensive GPU clusters. This design choice democratizes access to advanced congestion forecasting, lowering the barrier to entry for widespread industry adoption and enabling real-time deployment in resource-constrained trading environments.
