...
- **E5 — coverage-vs-width scatter** (LQR: too-narrow + under-covers; CASP: near-nominal).
...
- Calibration is **stable across seasons**: 5-seed windows (2026) show CASP best-calibrated in ALL — Jan-May (87.8%), Apr-Jul (92.7%), Jun-Aug (62.5%, all methods drop there) — while the ERROR winner flips by season (CASP / LQR / MLP). So the honest, defensible primary claim is calibration-first; error is competitive, not universally best.
- LQR's low error is purchased by being too narrow: in-sample 90% IW 7.85 with only 73% coverage, worst Winkler (26.20 vs CASP 20.72), and 17.8% quantile-crossing. LQR structurally lacks the constraint-attention mechanism CASP has (M11). E3 makes the trading consequence concrete without a full (overclaiming) backtest.
