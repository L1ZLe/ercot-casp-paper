...
- The full-year cross-year test (2025 -> 2026, ADR-0008) is a harsh full-year->half-year regime+season shift, where a simple linear baseline wins on OOD point error. Month-to-month keeps seasonal/regime structure roughly aligned and is a fairer measure of ``does SPARC generalize to similar near-range data.''
...
- Spawns TODO-16 (paper) note: report the near-range OOD (more favorable framing for SPARC) and the cross-year OOD (honest limitation: SPARC retains a calibration edge, no error advantage under regime shift).
