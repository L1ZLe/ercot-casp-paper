...
- **Market-rule-informed architecture**: To our knowledge, CASP is the first neural forecaster to condition a nodal day-ahead spread forecast directly on the ex-ante binding-constraint set (shadow prices, constraint IDs, flow ratios) posted by ERCOT, enabling the model to condition on the same information that drives congestion.
- **Competitive point accuracy and probabilistic loss**: CASP achieves the lowest MAE and RMSE among all compared methods on real ERCOT 2026 data, at average quantile loss equal to the best linear quantile regressor, with the gain appearing as calibration.
...
On the most extreme 5% of hours, tree methods reach a slightly lower spike MAE than CASP. We view this as the expected cost of our calibration focus; the cost to tail sharpness is a real limitation, and improving it is future work.
...
We evaluate CASP on one high-volume point-to-point pair, HB_HUBAVG to HB_PAN, over one year of ERCOT 2026 data. We chose this pair on purpose. It has the most traded volume, so it is the pair where hedging value is highest. The one-pair design gives us a clean controlled test of the constraint-attention idea. We show that clean test here and plan to extend it to more pairs and years next.
...
On raw pinball loss, CASP (AQL 1.335) and the best linear baseline (1.315) are close and not significantly different (p ≈ 0.09); we report this as a known tradeoff. Linear quantile regression is powerful when the target has exploitable linear structure, and its low AQL arises from under-coverage: it attains 73.2% success at an average 90% interval width of only 7.85, below the naive spread itself. CASP earns comparable AQL at 87.9% coverage with a 14.23 width. For a trader sizing a point-to-point position from the 0.10/0.90 quantiles, the difference between 73.2% and 87.9% coverage is the difference between routinely being hit by moves the interval was meant to bound and rarely being so.

The feedforward design keeps inference fast on CPU, which is a practical benefit. A richer temporal encoder is a direction for future work.
