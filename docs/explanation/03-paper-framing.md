...
- **Overall 0.10–0.90 coverage:** SPARC **87.9%** vs LQR 73.2%, MLP 84.4% (success_rate).
- **Width-aware calibration (Winkler-90):** SPARC **20.72** < LQR 26.20 < MLP 24.71 — best on the proper scoring rule that penalizes width, so the gain cannot be "just a wider box."
- **Significance:** success_rate vs LQR is significant (paired t p=0.003; Wilcoxon 0.062).
- **Coherence (AQCR):** SPARC **0.51%** crossing vs LQR 17.79%, MLP 8.58% — SPARC keeps quantiles ordered.
...
- **Efficiency:** SPARC **8,978** trainable params vs MLP 64,456 / LSTM 34,952 (~7x / ~4x smaller) at comparable-or-better accuracy — an efficiency win that mirrors the anchor paper.

**Claims we DO NOT make (scope out; do not write them):**
- ❌ **Do NOT claim "tail / spike coverage is the best."** MLP has higher spike-hour interval coverage (81.8% vs SPARC 55.9%); we simply do not make a tail-coverage claim. If tail is mentioned at all, scope it to "overall coverage" and do not quantify spike-coverage as a win.
...
- ❌ Do NOT write the row we lose as if we win it (e.g., don't present spike_mae as a SPARC advantage; trees/MLP beat us there).
...
- **Conformalized LQR (M8) — report once, in the discussion/limitations:**
  > "A split-conformalized quantile-regression wrapper raises the linear baseline's coverage from about 73% to 79%, yet still under-covers the nominal 90% target; SPARC's raw coverage (81%) remains above the conformalized baseline on the same test half."
...
- **AQL parity with LQR — phrase as an expected, honest trade:**
  > "SPARC is statistically indistinguishable from the linear benchmark on average quantile loss (p≈0.20) while delivering significantly higher calibration — the interval reliability a hedger needs — at a fraction of the parameters."

- **CRPS slightly favoring LQR — frame as consistency, not a hole:**
  > "CRPS is comparable between SPARC and the linear baseline (2.20 vs 2.15), consistent with the point-error parity; the decision-relevant improvement appears in coverage and Winkler."
...
| Window (2026) | SPARC succ% | LQR succ% | SPARC AQL | LQR AQL | best-error method |
|---|---|---|---|---|---|
| Jan–May | **87.8** | 86.9 | 2.124 | 2.343 | SPARC |
| Apr–Jul | **92.7** | 88.0 | 2.090 | 1.994 | LQR |
| Jun–Aug | **62.5** | 59.3 | 1.556 | 1.556 | MLP |

**Reading (honest):** SPARC is best-calibrated in ALL three seasonal windows, but the error winner flips by season (SPARC/LQR/MLP). So the paper claims **calibration stability across seasons** — never "best error across seasons." The Jun–Aug drop in all coverage is a harder/narrower window, not a SPARC-specific failure.
...
**Reading (honest):** inter-year drift (2025→2026 same months) is hard for everyone. SPARC has the **best calibration (84.9%)**, but MLP wins on error (AQL/MAE) and LQR is competitive — so the cross-year calendar test supports **calibration-transfer**, not error-transfer. Report as a stated limitation/edge, per ADR-0010.
