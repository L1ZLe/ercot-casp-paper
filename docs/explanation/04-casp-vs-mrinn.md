...
- **SPARC conditions on market-clearing outputs.** ERCOT day-ahead LMPs are *not* produced by a closed-form rule — they emerge from a large SCED optimization that co-determines prices, binding constraints, and shadow prices. There is no formula to embed. SPARC instead conditions on what the clearing *publishes* (binding-constraint identities + shadow prices + flow ratios, ex-ante under FERC Order 881), and learns the congestion-residual mapping with a constraint-attention layer.
...
4. Objective: **AQL** (pure pinball), quantile grid {0.10, 0.25, 0.45, 0.50, 0.55, 0.75, 0.90} — the same grid SPARC uses.
...
*Fidelity note:* [paper.tex](../../AIstats%20research%20paper/paper.tex) ($ paper source) describes a "hierarchical non-crossing quantile head"; the shipped implementation in [code/models.py](../../code/models.py) is a plain head + soft penalty. This doc describes the implemented mechanism, matching ADR-0004.
...
| Dimension | MRINN (Yu et al. 2026) | SPARC (this repo) |
|---|---|---|
...
- **Multi-year protocol.** 3-fold rolling over 2022–2025 vs SPARC's single-year chronological 70/15/15 (SPARC's cross-year OOD runs [ADR-0008](../adr/0008-cross-year-ood-and-modern-deep-baseline.md) partially compensate).

---

## 8. Caveats — what this comparison does NOT claim

- Does **not** claim SPARC beats MRINN on any numeric metric — the units, markets, and targets are not comparable.
- Does **not** claim tail/spike-coverage superiority for SPARC (MLP and trees beat SPARC on spike metrics; see [03-paper-framing.md](03-paper-framing.md)).
...
- Does **not** describe SPARC's head as "hierarchical" (implementation is a plain head + soft LA-CASF penalty).
- Does **not** claim cross-market generalization for SPARC — the architecture is market-agnostic, but its evaluation is single-market, single-year, single-pair (per research_brief limitations and [ADR-0010](../adr/0010-frequent-retraining-reframes-ood.md)).
