...
The primary dataset consists of ERCOT day‑ahead market clearing results at hourly resolution, sourced from the ERCOT Market Information System (MIS) public archive. The target variable is the spread between locational marginal prices at two liquid trading hubs: $s_t = \text{LMP}_{\text{HB\_PAN},t} - \text{LMP}_{\text{HB\_HUBAVG},t}$. HB_HUBAVG represents an averaged price across the Houston zone; HB_PAN is a specific node in the Panhandle region. This corridor experiences frequent congestion due to wind generation export from West Texas toward load centers, making it a representative high‑volume spread for hedging applications. Two additional node pairs (HB_HUBAVG to HB_NORTH and HB_HUBAVG to HB_WEST) serve as generalization probes, evaluated with the proposed method only (5 seeds each).
...
Cross-year evaluation (2025 training, 2026 testing) serves as a robustness stress test. In the calendar-aligned condition (January–June 2025 to January–June 2026), run at five seeds, SPARC achieves 84.9% coverage versus LQR's 80.6%, maintaining a 4.3-point advantage. In the full cross-year condition (full 2025 to full 2026), run at two seeds, both methods degrade: SPARC to 69.7%, LQR to 58.6%, with the 11.1-point relative advantage persisting. Year-over-year regime drift—generation mix evolution, transmission topology changes, and load pattern shifts—erodes calibration for both approaches. Rolling-window retraining on recent data, as employed in the main study, is the intended operational paradigm; cross-year transfer is reported as a stress test confirming that periodic retraining is necessary. This need for periodic re-fitting is well documented in the energy-forecasting concept-drift literature, where distribution shifts (seasonality, weather, evolving grid operations) degrade models over time and motivate adaptive re-training [hilger2023normalizing]. SPARC’s 8,978-parameter, CPU-minute training makes such frequent (monthly or seasonal, rather than annual) retraining operationally trivial, so the cross-year drift documented here is not a practical limitation but the very reason cheap, frequent re-fitting is the intended operating paradigm.

**Generalization probes.** To test whether the method transfers beyond the primary corridor, we ran the proposed method (5 seeds each) on two additional settlement pairs, HB_HUBAVG–HB_NORTH and HB_HUBAVG–HB_WEST. SPARC achieves 86.6% coverage (90% nominal) on the NORTH pair and 91.0% on the WEST pair — both in the same reliability range as, and bracketing, the primary pair’s 87.9%. This confirms that constraint-attention extends beyond the single corridor with consistent coverage across all three settlement points; these are 5-seed probes rather than full studies because baselines and ablations were not run on these pairs.
...
- **Per-seed outputs**: 334 artifacts (prediction, target, attention, and shift-factor \texttt{.npy} arrays) for every condition and seed on the primary pair, plus 5-seed generalization-probe outputs and per-seed arrays on the two additional pairs.
